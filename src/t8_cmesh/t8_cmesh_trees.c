...
t8_cmesh_trees_finish_part (t8_cmesh_trees_t trees, int proc)
{
  ...
  t8_debugf("ghost_attr_bytes: %zu\n", ghost_attr_bytes);
  t8_debugf("tree_attr_bytes: %zu\n", tree_attr_bytes);
  t8_debugf("face_neigh_bytes: %zu\n", face_neigh_bytes);
  t8_debugf("first_face: %zu\n", first_face);
  t8_debugf("first_ghost: %zu\n", first_ghost);
  t8_debugf("total_allocated: %zu\n", first_face + attr_bytes + face_neigh_bytes);
  t8_debugf("sizeof(ctree_t): %zu\n", sizeof(t8_ctree_struct_t));
  t8_debugf("sizeof(cghost_t): %zu\n", sizeof(t8_cghost_struct_t));
  t8_debugf("sizeof(ttf_ghost): %zu\n", sizeof (t8_gloidx_t) + sizeof (int8_t));
  t8_debugf("sizeof(ttf_tree): %zu\n", sizeof (t8_locidx_t) + sizeof (int8_t));
  t8_debugf("sizeof(t8_attribute_info_struct_t): %zu\n", sizeof(t8_attribute_info_struct_t));
  t8_debugf("sizeof(vertex_data): %zu\n", 3*sizeof (double));
...
t8_cmesh_trees_add_ghost_attribute (t8_cmesh_trees_t trees, int proc,
                              t8_stash_attribute_struct_t *attr,
                              t8_locidx_t local_ghost_id, size_t index, size_t *attribute_data_offset_from_beginning_of_attribute_descriptors)
{
  t8_part_tree_t      part;
  t8_cghost_t         ghost;
  char               *new_attr_data;
  ...
  attr_info->attribute_offset = *attribute_data_offset_from_beginning_of_attribute_descriptors - local_ghost_id * sizeof(t8_attribute_info_struct_t);
  new_attr_data = T8_GHOST_ATTR (ghost, attr_info);

  memcpy (new_attr_data, attr->attr_data, attr->attr_size);

  t8_debugf("\n\nEnter t8_cmesh_trees_add_ghost_attribute: \n");
  t8_debugf("local_ghost_id: %i\n", local_ghost_id);
  t8_debugf("index: %zu\n", index);
  t8_debugf("attribute_data_offset_from_beginning_of_attribute_descriptors: %zu\n", *attribute_data_offset_from_beginning_of_attribute_descriptors);
  t8_debugf("attr->attr_size: %zu\n", attr->attr_size);
#if T8_ENABLE_DEBUG
  double *vertices;
  vertices = (double *)T8_GHOST_ATTR(ghost, attr_info);
  t8_debugf("vertices[0] %f\n", vertices[0]);
#endif
  if(((char*)(part->first_tree))[928] != (char) -2){
    T8_ASSERT(0);
  }
  /* Set new values */
  attr_info->key = attr->key;
  ...
  *attribute_data_offset_from_beginning_of_attribute_descriptors += attr->attr_size;
