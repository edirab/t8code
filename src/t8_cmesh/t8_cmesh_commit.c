...
static void
t8_cmesh_add_attributes (t8_cmesh_t cmesh, sc_hash_t *ghost_ids, size_t *attribute_data_offset_from_beginning_of_attribute_descriptors)
{
    ...
    if (cmesh->first_tree <= attribute->id &&
      ...
      if (sc_hash_lookup (ghost_ids, temp_facejoin,
        ...
        t8_cmesh_trees_add_ghost_attribute (cmesh->trees, 0, attribute,
                                  (*facejoin_pp)->local_id, sj, attribute_data_offset_from_beginning_of_attribute_descriptors);
      }
    }
  }
...
t8_cmesh_commit_partitioned_new (t8_cmesh_t cmesh, sc_MPI_Comm comm)
{
  ...
  int                 F;
  size_t              si;
  size_t              attribute_data_offset_from_beginning_of_attribute_descriptors;
  ...
  t8_ghost_facejoin_t *ghost_facejoin = NULL, *temp_facejoin,
    **facejoin_pp;
  ...
  if (cmesh->num_local_trees != 0 || cmesh->num_ghosts != 0) {
    ...
    attribute_data_offset_from_beginning_of_attribute_descriptors = t8_cmesh_trees_finish_part (cmesh->trees, 0);
    t8_cmesh_trees_set_all_boundary (cmesh, cmesh->trees);
    ...
    t8_stash_attribute_sort (cmesh->stash);
    t8_cmesh_add_attributes (cmesh, ghost_ids, &attribute_data_offset_from_beginning_of_attribute_descriptors);
